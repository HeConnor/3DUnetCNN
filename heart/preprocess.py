"""
Tools for converting, normalizing, and fixing the heart data.
"""
...
from heart.train import config
...
def get_background_mask(in_folder, out_file, truth_name="GlistrBoost_ManuallyCorrected"):
    ...
    :param in_folder: a subject folder from the HEART dataset.
...
def get_image(subject_folder, name):
    file_card = os.path.join(subject_folder, name + "_frame_??.nii.gz")
    try:
...
def convert_heart_folder(in_folder, out_folder, truth_name='seg', no_bias_correction_modalities=None):
    name = os.path.basename(in_folder)
    try:
...
def convert_heart_data(heart_folder, out_folder, overwrite=False, no_bias_correction_modalities=("cine",)):
    """
    Preprocesses the HEART data and writes it to a given output folder. Assumes the original folder structure.
    :param heart_folder: folder containing the original heart data
    ...
    :return:
    """
    for subject_folder in glob.glob(os.path.join(heart_folder, "patient*")):
        if os.path.isdir(subject_folder):
            ...
            if not os.path.exists(new_subject_folder) or overwrite:
                ...
                convert_heart_folder(subject_folder, new_subject_folder,
                                     no_bias_correction_modalities=no_bias_correction_modalities)

if __name__ == '__main__':
    input_folder = '/media/heshengji/Ubuntu/CardiacData/ACDC2017/training'
    output_folder = 'data/preprocessed'
    convert_heart_data(input_folder, output_folder)
